...
RUN pip install \
    ...
    azul-plugin-mobsf==$(cd /tmp/src && python -m setuptools_scm)
...
RUN if [ "$GIT_BRANCH_NAME" = "refs/heads/dev" ] ; then \
    pip install --find-links /tmp/ azul-plugin-mobsf==$(cd /tmp/src && python -m setuptools_scm);fi
...
RUN python -m pytest --tb=short /tmp/tests
